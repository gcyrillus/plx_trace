<?php if(!defined('PLX_ROOT')) exit;
	...
	if(isset($_GET['dir'])) {
		...
		if (!file_exists(PLX_PLUGINS.basename(__DIR__).'/gpx/'.$dir)) {
			mkdir(PLX_PLUGINS.basename(__DIR__).'/gpx/'.$dir, 0777);
		}
		...
		if ($do == "rename") {
			...
			$increment = ''; 
			while(file_exists(PLX_PLUGINS.basename(__DIR__).'/gpx/'.$dir.'/'.$name . $increment . '.' . $arr_file_ext[0])) {
		...
		move_uploaded_file($_FILES['file']['tmp_name'], PLX_PLUGINS.basename(__DIR__).'/gpx/'.$dir.'/'.$newfilename);
